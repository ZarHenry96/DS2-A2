...
\begin{document}
	...
	\subsection{Plotting script usage}
		The plots for the performed analyses has been generated with the \texttt{chord\_analyzer.py} script, which was implemented for this purpose. The arguments usage is explained in \Cref{tab:script}. A valid script execution is the following: \\
		...
		\begin{table}[!h]
			...
			\begin{tabular}{cL{0.8\columnwidth}}
				...
				\hline
				2 & Requests for the scripts, comma separated. \texttt{loadnodes} and \texttt{loadlookups} respectively loads the Lookup and Node datasets. \texttt{all} generates all the implemented plots possible with the loaded datasets, which can also be selectively chosen with the corresponding names (see \Cref{tab:plots})\\
				\hline
				3 & Log files index \texttt{XX} to load, comma separated. Expecting \textit{Lookup\_XX} and \textit{Node\_XX} file as data sinks names\\
				\hline
